...
You can set environment variables for the duration of a Nushell session using [`let-env`](/commands/commands/let-env.html) calls inside the `env.nu` file. For example:
...
This will print out [`let-env`](/commands/commands/let-env.html) lines, one for each environment variable along with its setting.
...
As Nushell has its own [`open`](/commands/commands/open.md) command which has different semantics and shadows `/usr/bin/open`, these tools will error out when trying to use it.
...
In Nushell, [the PATH environment variable](<https://en.wikipedia.org/wiki/PATH_(variable)>) (Path on Windows) is a list of paths. To append a new path to it, you can use [`let-env`](/commands/commands/let-env.html) and [`append`](/commands/commands/append.html) in `env.nu`:

```
let-env PATH = ($env.PATH | split row (char esep) | append '/some/path')
```

This will append `/some/path` to the end of PATH; you can also use [`prepend`](/commands/commands/prepend.html) to add entries to the start of PATH.

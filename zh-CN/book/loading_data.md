...
之前我们使用了[`ls`](/zh-CN/commands/docs/ls.md)、[`ps`](/zh-CN/commands/docs/ps.md)、[`date`](/zh-CN/commands/docs/date.md)和[`sys`](/zh-CN/commands/docs/sys.md)等命令来加载关于文件、进程、日期时间和系统本身的信息。每条命令都会给我们提供一个信息表，以对其进行探索。我们也可以通过其他方式将数据载入表格以供使用。

## 打开文件

Nu 在处理数据方面最强大的能力之一是[`open`](/zh-CN/commands/docs/open.md)命令。它是一个多功能命令，可以处理许多不同的数据格式。为了说明这一点让我们试着打开一个 JSON 文件：

@[code](@snippets/loading_data/vscode.sh)

与[`ls`](/zh-CN/commands/docs/ls.md)类似，打开一个 Nu 支持的文件类型，会返回一些不仅仅是文本（或一个字节流）的东西。这里我们打开了一个来自 JavaScript 项目的 "package.json" 文件。Nu 可以识别 JSON 文本并将其解析为一个数据表。

如果我们想查看当前项目的版本，我们可以使用[`get`](/zh-CN/commands/docs/get.md)命令：
...
可以看到，我们正在处理这些行，因为我们又回到了一个列表中。下一步是看看是否可以把行分割成更有用的东西。为此，我们将使用[`split`](/zh-CN/commands/docs/split.md)命令。[`split`](/zh-CN/commands/docs/split.md)，顾名思义，为我们提供了一种分割字符串的方法。我们将使用[`split`](/zh-CN/commands/docs/split.md)的`column`子命令，将内容分成多列。我们会告诉它分隔符是什么，剩下的就由它来完成：
...
这看起来差不多了，只是还有一些额外的空白字符，让我们 [`trim`](/zh-CN/commands/docs/str_trim.md) 掉这些空格：
...
还不错，[`split`](/zh-CN/commands/docs/split.md)命令返回给我们可以使用的数据，还预设了默认的列名：
...
- [`str`](/zh-CN/commands/docs/str.md)
- [`lines`](/zh-CN/commands/docs/lines.md)
...
虽然能够打开一个文件并立即使用其数据表很有帮助，但这并不总是我们想要的。为了获得原始文本，[`open`](/zh-CN/commands/docs/open.md)命令可以接受一个可选的`--raw`标志：
...
SQLite 数据库会被 [`open`](/zh-CN/commands/docs/open.md) 自动检测，无论其文件扩展名是什么。你可以打开整个数据库：

```nu
open foo.db
```

或者 [`get`](/zh-CN/commands/docs/get.md) 一个特定的表：
...
除了从文件系统中加载文件，你还可以通过使用[`http get`](/zh-CN/commands/docs/http.md)命令来加载 URLs。这将从互联网上获取 URL 的内容并返回：

...
另见 [字符串的使用](https://www.nushell.sh/book/loading_data.html#working-with-strings)。
...
最常见的模式是 "_"，它将匹配所有的路径。通常，你会看到这个模式被用作另一个模式的一部分，例如`_.bak`和`temp\*`。
...
文件路径(File paths)是在给定的操作系统中代表文件路径的一种独立于平台的方式。比如`/usr/bin`和`C:\Users\file.txt`。

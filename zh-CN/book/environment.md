...
最后的`raw`列显示了将被发送到外部应用程序的实际值（详见[环境变量转换](#环境变量转换)）。
...
如果你有一个以上的环境变量需要设置，你可以使用`load-env`来创建一个键/值对记录(Record)，并用于加载多个环境变量：
...
如果你想删除源自父级作用域的环境变量，你可以用[`hide`](/book/commands/hide.html)隐藏它（关于隐藏的细节，请参见[模块](modules.md)）：

...
Nushell 将尊重并使用 Mac、Linux 和 Windows 上的 `LS_COLORS` 环境变量设置。这个设置允许你在做[`ls`](/book/commands/ls.md)时定义文件类型的颜色。例如，你可以让目录变成一种颜色，`_.md` markdown 文件一种颜色，`_.toml` 文件变成另一种颜色，等等。有很多方法可以给你的文件类型着色。
...
如果你想在主题设计上全力以赴，你需要把我在一开始提到的所有项目作为主题，包括`LS_COLORS`和提示。祝您好运!

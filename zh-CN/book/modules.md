...
与[`def`](/book/commands/def.md)类似，也可以用`export`关键字标记[`def-env`](/book/commands/def-env.md)（你可以在[环境](environment.md)章节中了解更多关于[`def-env`](/book/commands/def-env.md)的信息）。

## 使用模块

模块本身并不做任何事情，要使用模块导出的定义，我们需要[`use`](/book/commands/use.md)它：
...
一般来说，[`use`](/book/commands/use.md)关键词后面的任何内容都会形成一个**导入模式**，它控制着符号的导入方式。
导入模式可以是以下的一种：
...
现在，你可以直接在文件上调用[`use`](/book/commands/use.md)：
...
其语法与你可能习惯的[`let-env`](/book/commands/let-env.md)或[`load-env`](/book/commands/load-env.md)等命令略有不同：
...
你可能注意到我们没有直接给`MYNAME`赋值，相反，我们给了它一个代码块（`{ ...}`），它在我们每次调用[`use`](/book/commands/use.md)时都会被执行。例如，我们可以用[`random`](/book/commands/random.md)命令来演示这一点：
...
我们用[`hide`](/book/commands/hide.md)命令来实现隐藏：
...
[`hide`](/book/commands/hide.md)命令也接受导入模式，就像[`use`](/book/commands/use.md)那样。不过，导入模式的解释略有不同。它可以是下面中的一种：

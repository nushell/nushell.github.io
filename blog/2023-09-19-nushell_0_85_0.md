...
- `std from ndjson` as a way to directly open newline-delimited JSON files. You can use `open` for files with the `.ndjson` ending if you `use std from ndjson`.
- `std from jsonl` as a way to directly open newline-delimited JSON files. You can use `open` for files with the `.jsonl` ending if you `use std from jsonl`.

...
Nushell now gives more specific (and hopefully more helpful) messages when it detects syntax errors in the commmand line.  Currently, these messages apply only to **string** and **int** literals.  But this work will be extended to more aspects of the language in future releases.
...
The error description is more specific and the underlined span is better localized.  

### Int literals

Some checks are available for int literals also.
<table>
<tr>
<th>Before</th>
<th>After</th>
 ...
 ```
------------------------------------------------------
# invalid integer literals with various radices
------------------------------------------------------
〉( 0 + 0x22 )      # Good hex int literal
34
...
------------------------------------------------------
〉( 0 + 0b01bogus ) # bad Binary int literal
Error: nu::parser::unsupported_operation (link)

  × Types mismatched for operation.
   ╭─[entry #27:1:1]
 1 │ ( 0 + 0b01bogus ) # bad Binary int literal
   ·   ┬ ┬ ────┬────
   ·   │ │     ╰── string
   ·   │ ╰── doesn't support these values.
   ·   ╰── int
...
------------------------------------------------------
〉( 0 + 0o67bogus ) # bad Octal int literal
Error: nu::parser::unsupported_operation (link)

  × Types mismatched for operation.
   ╭─[entry #28:1:1]
 1 │ ( 0 + 0o67bogus ) # bad Octal int literal
   ·   ┬ ┬ ────┬────
   ·   │ │     ╰── string
   ·   │ ╰── doesn't support these values.
   ·   ╰── int
...
Error: 
 ...
 1 │ ( 0 + 0b01bogus ) # bad Binary int literal
   ·       ────┬────
   ·           ╰── invalid digits for radix 2 in int
   ╰────
...
Error: 
 ...
 1 │ ( 0 + 0o67bogus ) # bad Octal int literal
   ·       ────┬────
   ·           ╰── invalid digits for radix 8 in int
   ╰────
...
Specifically, **decimal int** and **float** literals operate the same as ever.  These  lack clear signals of user intent that enclosing quotes or `0x` prefix provide for the above literals.  This makes it trickier to decide whether to terminate the parse and display an error message or try another interpretation of the token, such as a bare string or (with leading minus sign) a command flag.
<table>
<tr>
<th>Unchanged</th>
 </tr>
...
This is an active are of research and we hope to produce more results!

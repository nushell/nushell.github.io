...
This release includes major changes how environment is handled in respect to modules.
...
Since Nushell is a "compiled" language where commands and aliases are brought into the scope and compile time, sourcing dynamic paths is not possible for the same reason you cannot dynamically set `#include` file names in C or `use`d modules in Rust, for example.
You can read a bit more about this in [our old book](https://web.archive.org/web/20220905200522/https://www.nushell.sh/book/thinking_in_nu.html#think-of-nushell-as-a-compiled-language).
...
_How do you bring in commands and aliases, then?_ You need to use a module and the `use` keyword. See our book [about modules](https://www.nushell.sh/book/modules.html). Alternatively, you can use [overlays](https://www.nushell.sh/book/overlays.html).

_How do you bring in variables, then?_ You cannot. A workaround is to define a command in your module that will return the value you want.
...
Since 0.67, it is possible to [use modules within other modules](https://github.com/nushell/nushell/pull/6162) but because environment is 100% handled in runtime, and `use` never evaluates the module itself, it was impossible to bring in environment variables from other module inside a module.
...
To deal with environment variables from modules, instead of defining them with `export env` individually, there is a single `export-env { }` block for the whole module.
If you call `source-env` on that module, that block will get evaluated and its environment brought in:
```
# spam.nu
...
In this release, we allowed `export` keywords to be used in scripts (they do not do anything) and thus **script syntax is a superset of module syntax**.
(The only exception is `export env` which is deprecated and will be removed in 0.69.)

This means that any module can now be evaluated, which is the reason the above solution with `source-env` works: The `export-env` is just a command that acts as `do` but preserves the environment:
You can call it also manually outside of modules:
```
> export-env {
    let-env FOO = 'foo'
}
...
Another nice thing about the unified syntax is that commands like `nu-highlight` now do not break if they are asked to parse a module code.

### Overlays

`overlay add` and `overlay remove` are now renamed to `overlay use` and `overlay hide` (see the breaking changes later).

The functionality of these commands remains largely the same with one change being that `overlay use` will now evaluate the `export-env { }` block, like `source-env`.
...
> overlay use spam.nu
```
You can think of `overlay use` as calling `source-env` and `use` in one command and putting it into a new overlay.
...
For example other commands returning a boolean value like `str contains` never carried the suffix.
To avoid potential sources of confusion and free up the `?` for potential uses in a more meaningful semantic context, we decided to remove the suffix and rename `empty?` to `is-empty` to make its role more clear.

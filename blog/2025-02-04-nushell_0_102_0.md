...
excerpt: Today, we're releasing version 0.102.0 of Nu. This release adds runtime pipeline input type checking,
...
In addition, you can now control how many decimal places filesizes should be formatted with using `$env.config.filesize.precision`. When set to `null`, as many decimal places as necessary will be printed. Otherwise, when set to a integer, that number of decimal places will be shown.

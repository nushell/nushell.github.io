...
## Empty lists now handled differently ([Benjamin-L](https://github.com/nushell/nushell/pull/8669), [rgwood](https://github.com/nushell/nushell/pull/8651))

This release includes several changes to make handling of empty lists/tables more consistent:

- `[] | select foo` now returns an empty list instead of `null`
- `sort`, `uniq`, `sort-by`, and `uniq-by` now return empty lists when given an empty list (previously they would throw an error)

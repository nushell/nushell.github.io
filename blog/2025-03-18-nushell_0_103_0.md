...
### `from xml --disallow-dtd`

After [#15272](https://github.com/nushell/nushell/pull/15272), document type declarations (DTD) are now parsed by default when using `from xml`. To disallow DTDs and instead error like before, you can pass the new `--disallow-dtd` flag.
...
A new config option, `$env.config.filesize.show_unit`, was introducted in [#15276](https://github.com/nushell/nushell/pull/15276). `show_unit` is `true` by default, but when it is set to `false`, file sizes will be displayed without units (e.g., in `table`). This can be useful if you have set `filesize.unit` to a fixed unit, and you do not want the same unit to be shown over and over again.

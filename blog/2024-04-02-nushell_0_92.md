...
As part of this effort, we now stream external command output in more places where it is possible.
Namely, parentheses no longer collect external command output and are now soley for grouping and precedence.
...
So, external commands will now only be collected if it's being turned into a value
(or if it's passed to an internal commands that collect its input).

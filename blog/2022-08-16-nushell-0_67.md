...
Sometimes, however, you might want them behind the `spam` prefix.
That's what `overlay add --prefix spam` is for.
Here is an example:
```
> module spam { 
    export def foo [] { "foo" }
...
It can be useful, for example, when using the [conda module](https://github.com/nushell/nu_scripts/blob/main/virtual_environments/conda.nu) as an overlay so you'll get commands named `conda activate` instead of just `activate`.

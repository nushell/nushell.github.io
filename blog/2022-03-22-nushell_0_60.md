...
| filters      | `columns`, `group`, `par-each`, `transpose`, `window`, `roll`, `roll down`, `roll left`, `roll right`     |
...
Previous versions of Nushell struggled with a very common pattern: errors that happen in code that isn't the code you just typed. For example, aliases that failed, calls to custom commands that were previously defined, and more.

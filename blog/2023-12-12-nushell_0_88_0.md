...
[#10598](https://github.com/nushell/nushell/issues/10598) asked for a spread operator, and [#11006](https://github.com/nushell/nushell/pull/11006)
and [#11144](https://github.com/nushell/nushell/pull/11144/) implemented a spread operator in lists and records, respectively.
The spread operator can help one avoid `append` (when used in lists) and `merge` (when used in records). Spreading arguments to commands has not been implemented yet.

```nushell
> let x = ["foo"]; [ 1 ...$x ...("foo" | split chars) ...[3 9] [] ] | to nuon
[1, foo, f, o, o, 3, 9, []]
> let x = { a: "foo" }; { ...$x y: "bar" ...{ x: 2 } } | to nuon
{a: foo, y: bar, x: 2}
```

You can spread variables, subexpressions, and either lists (inside lists) or records (inside records).

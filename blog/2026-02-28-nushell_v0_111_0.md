...
nushell will use `pipefail` by default, so something like `^false | lines` returns an empty list, with a failure exit status code
...
Removed `$env.config.input_list` config, instead sensible defaults are inherited from `color_config`.
...
Added tutor for `closure` (and updated `block` tutor).
...
This release adds the `umask` command, which lets you control the default permissions for newly-created files and directories.
...
External command tab completion on Windows now includes PowerShell `.ps1` scripts that are available in `PATH`.
...
Enabling `shell_integration.osc133` now also enables click-to-cursor in supported terminals.
...
Added `--left` flag to `drop column` command to drop columns on the left side instead of the right side.
...
Ctrl+C now immediately interrupts `http get` (and other HTTP commands) when waiting for slow or streaming responses. Previously, Ctrl+C was ignored until data arrived. This makes it practical to wrap long-polling HTTP APIs with nushell pipelines.

`du` command will now also show colorful paths and icons like `ls`
...
Disable ANSI coloring if `TERM` is set to `"dumb"` when `$env.config.use_ansi_coloring` is set to `"auto"`.
...
`select` is now documented as the retain operation, `help --find retain` points to it, and `reject help` directs users to select for the inverse behavior.
...
format filesize now uses $env.config.float_precision to control decimal places for fractional values.
...
Improves help text for filter commands (e.g. each, select, where) with clearer, consistent descriptions.
...
This is PR 5 (out of 10 total smaller PRs for issue 5066) improves command descriptions, flag descriptions, and example descriptions for bytes, conversions, database, and date commands in `crates/nu-command/src/`, as part of [Issue #5066](https://github.com/nushell/nushell/issues/5066) — "Help us with better command and parameter/flag descriptions."

Help text for filesystem, path, and platform commands (`cd`, `ls`, `open`, `rm`, `path join`, `path exists`, `clear`, `term size`, `whoami`, and related commands) is now more consistent and easier to read: command, flag, and example descriptions use consistent capitalization and punctuation. Behavior of these commands is unchanged.

This PR 9 (of 10 for Issue 5066) improves command descriptions, flag descriptions, and example descriptions for string-related commands in `crates/nu-command/src/strings/`, as part of [Issue #5066](https://github.com/nushell/nushell/issues/5066) — "Help us with better command and parameter/flag descriptions."
...
All environment variable names are now case-insensitive for lookups and case-preserving for storage on all operating systems.
...
Improves command and parameter/flag descriptions for the network, system, and viewers modules (Issue #5066).

Improved command and parameter/flag descriptions for consistency.
...
`ls` now escapes control characters in filenames instead of passing them through to the terminal.

`history | last x` will be in ascending order.

The new built-in commands `clip copy` and `clip paste` are now behind the experimental option `native-clip`.
...
Remove `unlet` variables from completions.

Allow Swiss German keyboard, specifically `AltGr` keys, with `explore regex`

Allows `view source` to see if `--wrapped` or `--env` was used in the custom command and reconstruct it properly.

Fixed reading old plugin files to migrate.

...
Wie im anderen Kapitel bereits gezeigt wurde, ist [`ls`](/commands/commands/ls.md) der Befehl um den Inhalt eines Pfades anzuzeigen. Der Output wird als Tabelle dargestellt.

Der [`ls`](/commands/commands/ls.md) Befehl nimmt auch ein optionales Argument entgegen, um die Ansicht zu verändern. Zum Beispiel um nur ".md" Dateien an zu zeigen.
...
Um vom aktuellen Verzeichnis in ein neues zu gelangen, wird der [`cd`](/commands/commands/cd.md) Befehl verwendet. Wie in jeder anderen Shell, kann entweder der Name des Verzeichnisses angeben, oder mit `..` ein Verzeichnis hochgesprungen werden.

Es kann jedoch auch das Verzeichnis gewechselt werden ohne [`cd`](/commands/commands/cd.md) angeben zu müssen, indem einfach der neue Pfad angegeben wird:

@[code](@snippets/moving_around/cd_without_command_example.sh)

**Hinweis** wenn mit [`cd`](/commands/commands/cd.md) das Verzeichnis gewechselt wird, ändert sich die `PWD` Umgebungsvariable. Das heisst der Wechsel bleibt für den aktuellen Block bestehen. Beim Verlassen des Blocks, wird wieder ins vorherige Verzeichnis gewechselt.
...
Ein Objekt von einem Ort an einen anderen verschieben mit [`mv`](/commands/commands/mv.md):
...
Diese drei Befehle können auch von der glob Fähigkeit profitieren, wie beim [`ls`](/commands/commands/ls.md).

Zu guter Letzt wird ein neues Verzeichnis angelegt mit dem [`mkdir`](/commands/commands/mkdir.md) Befehl:

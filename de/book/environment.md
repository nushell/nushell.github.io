...
Die aktuellen Umbegungsvariablen werden mit dem [`env`](/commands/commands/env.html) Befehl ausgegeben:
...
### Aufruf eines Befehls mit [`def-env`](/commands/commands/def-env.md)
...
Etwas was in einer Shell häufig vorkommt ist das wechseln des Verzeichnisses mit dem [`cd`](/commands/commands/cd.html) Befehl.
...
Es kann auch [`with-env`](/commands/commands/with-env.html) verwendet werden um expliziter zu sein:
...
Der [`with-env`](/commands/commands/with-env.html) Befehl setzt die Umgebungsvariable temporär (hier wird die Variable "FOO" auf den Wert "BAR" gesetzt)
...
Wird sie jedoch mit [`dev-env`](/commands/commands/def-env.html) anstatt [`def`](/commands/commands/def.html) gesetzt, so wird sie beim verlassen des Blocks erhalten bleiben.
...
Zurückschauend auf den [`env`](/commands/commands/env.html) Befehl, die `raw` Spalte zeigt die Werte, die mit `ENV_CONVERSIONS.<name>.to_string` übersetzt wurden.
...
Umgebungsvariablen können im aktuellen Gültigkeitsbereich entfernt werden via [`hide`](/commands/commands/hide.html):
